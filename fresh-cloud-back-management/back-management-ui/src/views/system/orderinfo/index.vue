...
<script>
import { listOrderinfo, getOrderinfo, delOrderinfo, addOrderinfo, updateOrderinfo } from "@/api/system/orderinfo";
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.ono)
    ...
    handleUpdate(row) {
      this.reset();
      const ono = row.ono || this.ids
      getOrderinfo(ono).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const onos = row.ono || this.ids;
      this.$modal.confirm('是否确认删除订单信息编号为"' + onos + '"的数据项？').then(function() {
        return delOrderinfo(onos);
      }).then(() => {
        this.getList();
